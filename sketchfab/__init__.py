...
PLUGIN_VERSION = str((1, 6, 1)).strip('() ').replace(',', '.')
preview_collection = {}
...
class SketchfabApi:
    ...
    def login(self, email, password, api_token):
        bpy.ops.wm.sketchfab_login_modal('INVOKE_DEFAULT')
...
class SketchfabLoginModal(bpy.types.Operator):
    """Login into your account"""
    bl_idname = "wm.sketchfab_login_modal"
    bl_label = ""
    bl_options = {'INTERNAL'}
...
def upload(filepath, filename):
    ...
    if r.status_code not in [requests.codes.ok, requests.codes.created, requests.codes.no_content]:
        ...
        try:
            ...
            if len(folders) >= 3:
                print("base project_name=", folders[-3], "base room_name=", folders[-1].replace("-3d_export.blend","").replace("_"," "))
                project_name = folders[-3]
                room_name = folders[-1].lower().replace("-3d_export.blend","").replace("_"," ")
                    
            wm = bpy.context.window_manager.sn_project
            wm.project_index = wm.projects.find(project_name)
            ...
            for room in project.rooms:
                ...
                if room.name.lower() == room_name:
                    ...
                    room.prop_3d_exported_url = sf_state.model_url
                    room.prop_3d_exported_uid = result["uid"]
                
                    print("room.prop_3d_exported_url=", room.prop_3d_exported_url)
                    print("room.prop_3d_exported_uid=", room.prop_3d_exported_uid)
                    break
...
def check_plugin_version(request, *args, **kwargs):
    ...
    if response and len(response):
        ...
        current_version = str((1, 6, 1)).replace(',', '').replace('(', '').replace(')', '').replace(' ', '')
