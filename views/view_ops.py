...
class VIEW_OT_generate_2d_views(Operator):
    ...
    def create_linestyles(self):
        ...
        if self.is_kitchen_bath:
            hidden_linestyle.dash1 = self.HIDDEN_LINE_DASH_PX_KB
            hidden_linestyle.dash2 = self.HIDDEN_LINE_DASH_PX_KB
            hidden_linestyle.dash3 = self.HIDDEN_LINE_DASH_PX_KB
            hidden_linestyle.gap1 = self.HIDDEN_LINE_GAP_PX_KB
            hidden_linestyle.gap2 = self.HIDDEN_LINE_GAP_PX_KB
            hidden_linestyle.gap3 = self.HIDDEN_LINE_GAP_PX_KB
        else:
    ...
    def create_replaced_mesh(self, obj, materials=[], glass_color=None):
        ...
        if is_styled and "glass" in materials and cover_mesh:
            ...
            if glass_color:
                if glass_color == "Double Sided Mirror":
                    glass_color = "DS Mirror"
                glass_label.set_label(glass_color)
            else:
    ...
    def group_every_children_on_acc(self, grp, obj):
        ...
        shown = any([True for c in obj.children if c.type == 'MESH' and not c.hide_viewport])
        ...
        is_shelf_insert = obj.get('IS_BP_SHELVES')
        if is_shelf_insert:
            if sn_utils.get_cabinet_bp(obj):
                shelf_insert = sn_types.Assembly(obj)
                shelf_thickness_ppt = shelf_insert.get_prompt("Shelf Thickness")
                if shelf_thickness_ppt:
                    shelf_thickness_ppt.set_value(0)
    ...
    def empty_wall(self, wall):
        ...
        no_appliances = appliances == 0
        if no_sections and no_csh and no_lsh and no_acc and no_beds and no_isl and no_cabinets and no_appliances:
            return True
        return False
    ...
    def create_plan_view_scene(self, context):
        ...
        for obj in self.main_scene.objects:
            ...
            if obj.get("IS_BP_ISLAND") and not obj.get("IS_BP_CABINET"):
    ...
    def has_positive_overlay(self, item):
        ...
        wall_length_rd = round(unit.meter_to_inch(wall_length), 2)
        item_length_rd = round(unit.meter_to_inch(item_length), 2)
        item_loc_x_rd = round(unit.meter_to_inch(item_loc_x), 2)

        negative_x_loc = item_loc_x_rd < 0

        corner_shelves = item.get("IS_BP_CORNER_SHELVES") or item.get("IS_BP_L_SHELVES") or item.get("IS_CORNER")

        if corner_shelves:
            positive_x_dim = item_loc_x_rd > wall_length_rd
        else:
            positive_x_dim = item_loc_x_rd + item_length_rd > wall_length_rd
    ...
    def accordion_cross_section_slot(self, current_wall, previous_wall, next_wall, assemblies_dict):
        ...
        curr_wall_assy = sn_types.Assembly(current_wall)
        curr_wall_width = curr_wall_assy.obj_x.location.x
        ...
        if start_curr is not None and depth_prvs is not None:
            ...
            if depth_prvs <= start_curr and (start_situation or end_situation):
                ...
                # Check if there is a corner on the previous wall
                for obj in previous_wall.children:
                    distance_to_wall_end = None

                    if obj.get("IS_BP_L_SHELVES") or obj.get("IS_BP_CORNER_SHELVES"):
                        distance_to_wall_end = curr_wall_assy.obj_x.location.x - obj.location.x
                        ...
                        if acc_length_with_next_wall > break_width:
                            if next_wall:
                                if wall_width - corner.location.x < unit.inch(6):
                                    self.replace_append_corner_instances(targets, next_wall, corner_instances)
                                elif corner.location.x < unit.inch(6):
                                    self.replace_append_corner_instances(targets, wall, corner_instances)
                            else:
                                self.replace_append_corner_instances(targets, previous_wall, corner_instances)
                        else:
                            self.replace_append_corner_instances(targets, wall, corner_instances)
    ...
    def apply_kb_build_combo_height_label(self, wall_bp, base_position, upper_position, upper_height, light_rail_height, multiplier, ovrly_x_offset):

        # print("apply_kb_build_combo_height_label.base_position: " + str(base_position))
        # print("apply_kb_build_combo_height_label.upper_position: " + str(upper_position))
        # print("apply_kb_build_combo_height_label.upper_height: " + str(upper_height))
        # print("apply_kb_build_combo_height_label.light_rail_height: " + str(light_rail_height))

        # base cabinet height dim
        base_position_metric = base_position * unit.inch(1)
        ...
        dim.start_z(value=0)
        dim.end_z(value=base_position_metric)
        dim.set_label(str(base_position) + "\"")

        # empty space height dim
        empty_position = upper_position - upper_height - light_rail_height - base_position 
        empty_position_metric = empty_position * unit.inch(1)
        dim = sn_types.Dimension()
        dim.parent = wall_bp
        dim.start_x(value=offset + curr_wall_glb_x)
        dim.start_z(value=base_position_metric)
        dim.end_z(value=empty_position_metric)
        dim.set_label(str(round(empty_position,2)) + "\"")

        # upper cabinet height dim
        upper_position_metric = upper_position * unit.inch(1)
        upper_height_metric = upper_height * unit.inch(1)
        light_rail_height_metric = light_rail_height * unit.inch(1)
        dim = sn_types.Dimension()
        dim.parent = wall_bp
        dim.start_x(value=offset + curr_wall_glb_x)
        dim.start_z(value=base_position_metric + empty_position_metric)
        dim.end_z(value=upper_height_metric + light_rail_height_metric)
        dim.set_label(str(upper_height+light_rail_height) + "\"")
    ...
    def accordion_build_hight_n_ceiling_dimensions(self, context, accordion, existing_dims, extra_dims):
        ...
        upper_cabinet_height = 0
        upper_cabinet_position = 0
        light_rail_height = 0
        for i, wall_bp in enumerate(accordion):
            ...
            for item in wall_bp.children:
                if "IS_BP_CABINET" in item:
                    ...
                    # upper_cabinet_height = 0
                    # light_rail_height = 0
                    if item.get("CARCASS_TYPE") == "Upper":
                        upper_cabinet_height = math.fabs(assy.obj_z.location.z)
                        upper_cabinet_position = assy.obj_bp.location.z + ctop_thickness
                    # print("upper_cabinet_height", upper_cabinet_height)
                    ...
                    for sub_item in item.children:
                        ...
                        elif "IS_BP_LIGHT_RAIL" in sub_item and light_rail_height == 0:
                            sub_assy = sn_types.Assembly(sub_item)
                            light_rail_height = sub_assy.obj_y.location.y
                        # print("light_rail_height", light_rail_height)  
        ...
        if len(bh_dims) > 0:
            ...
            for i, value in enumerate(bh_dims):
                if i == 1 and upper_cabinet_height > 0:
                    upper_cabinet_position = self.to_inch(upper_cabinet_position)
                    upper_cabinet_height = self.to_inch(upper_cabinet_height)
                    light_rail_height = self.to_inch(light_rail_height)
                    self.apply_kb_build_combo_height_label(first_wall, value, upper_cabinet_position, upper_cabinet_height, light_rail_height, i, overlay_x_offset)
    ...
    def accordion_building_height_ceiling_height(self, context, accordion, walls_list, extra_dims, wall_rem_spaces):
        offsets = []
        exstg_dims = []
        ...
        for wall in accordion:
            ...
            offset = self.accordion_build_hight_n_ceiling_dimensions(context, accordion, exstg_dims, extra_dims)
    ...
    def make_kb_crown_label(self, labels, molding_bp):
        ...
        parent_assy = sn_types.Assembly(molding_bp.parent)
        if parent_assy:
            base_height = parent_assy.obj_bp.location.z 
            cabinet_height = parent_assy.obj_z.location.z
            if cabinet_height > base_height:
                base_height += cabinet_height

        molding_assy = sn_types.Assembly(molding_bp)
        label_name = molding_bp.name
        if label_name.find(".") > 0:
            label_name = label_name[:-4]
        if molding_assy:
            molding_height = molding_assy.obj_y.location.y
            # fc_str = f'{label_name} {self.to_inch_lbl(molding_height)}'  # Remove height from label until do molding size update work Issue TBD
            fc_str = label_name
            labels.append((fc_str, base_height + molding_height))
            ...
            for j, wall in enumerate(accordion):
                ...
                for item in wall_groups_list:
                    if 'door' in item.name.lower():
                        for door in item.children:
                            if 'door frame' in door.name.lower():
                                self.return_wall_labels(wall, door, False, True)

            offset = self.accordion_building_height_ceiling_height(
                context, accordion, walls_list, extra_dims, walls_rem_spaces)
    ...
    def add_meshes_to_freestyle_collections(self):
        for item in self.parts_for_freestyle():
            ...
            appliance_bp = sn_utils.get_appliance_bp(item)
            is_appliance = True if appliance_bp else False

            exclude_vis = any(in_vis) and not any(include_vix_override)
            exclude_hide = (any(in_hide) or is_kd or is_appliance) and not any(include_hide_override)
    ...
    def get_ct_assy_bp(self, obj_bp, ct_type):
        for child in obj_bp.children:
            if child.get(ct_type):
                return child
    ...
    def write_island_countertop(self, island, x_loc):
        ...
        if not is_cabinet:
            ...
            if no_CT:
                ...
                if CT_type == 'Wood':
                    countertop_bp = self.get_ct_assy_bp(island_assembly.obj_bp, "COUNTERTOP_WOOD")
                    if countertop_bp:
                        if countertop_bp.sn_closets.use_unique_material:
                            CT_info = CT_type + " " + countertop_bp.sn_closets.wood_countertop_types
                        else:
                    ...
                    countertop_bp = self.get_ct_assy_bp(island_assembly.obj_bp, "COUNTERTOP_QUARTZ")
                    if countertop_bp:
                        if countertop_bp.sn_closets.use_unique_material:
                            CT_info = CT_type + " " + countertop_bp.sn_closets.unique_countertop_quartz
                        else:
                    ...
                    if countertop_bp:
                        if countertop_bp.sn_closets.use_unique_material:
                            CT_info = CT_type + " " + countertop_bp.sn_closets.unique_countertop_standard_quartz
                        else:
                            CT_info = CT_type + " " + ct_mat_props.get_color().name

                elif CT_type == 'HPL':
                    countertop_bp = self.get_ct_assy_bp(island_assembly.obj_bp, "COUNTERTOP_HPL")
                    if countertop_bp:
                        if countertop_bp.sn_closets.use_unique_material:
                            CT_info = CT_type + " " + countertop_bp.sn_closets.unique_countertop_hpl
                        else:
                            CT_info = CT_type + " " + ct_mat_props.get_color().name
    ...
    def get_height_code(self, height):
        ...
        for item in front_heights:
            ...
            # print("height_inch: ", height_inch, "num_inch: ", num_inch, "item_inch: ", item_inch, "item: ", item)
    ...
    def write_door(self, obj, x_loc, z_loc):
        ...
        if is_cabinet:
            doors = sn_utils.get_tagged_bp_list(obj, 'IS_DOOR', doors)
            ...
            for door in doors:
                for child in door.children:
                    if child.get('DOOR_HOLE_LABEL'):
                        if child.snap.opengl_dim.gl_label != "":
                            label = child.snap.opengl_dim.gl_label
    ...
    def write_drawer(self, obj, x_loc, z_loc):
        ...
        if drawer_quantity:
            ...
            for i in range(range_start, 0, -1):
                ...
                if label == 0:
                    label = self.get_height_code(drawer_height + unit.inch(0.01))
    ...
    def write_island_chase(self, island, x_loc, view):
        ...
        #     return
        # else:
        if chase_depth and round(to_inch(chase_depth)) > 0:
            tk_height = carcass_assembly.get_prompt("Toe Kick Height").get_value()
            shelf_thk = carcass_assembly.get_prompt("Top Thickness").get_value()
            double_sided = carcass_assembly.get_prompt("Double Sided").get_value()
            ...
            if double_sided:
                ...
                chase_label.start_x(value=x_label)
