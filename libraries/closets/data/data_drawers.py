...
UPPER_DRW_PLACEHOLDER = [
    ('123.952', '4H-4.88"', '4H-4.88"'),
]

UPPER_DRW_PLACEHOLDER_LIST = ['4H']
...
class Drawer_Stack(sn_types.Assembly):
    ...
    def add_drawer_box_prompts(self, drawer_num):
        ...
        self.add_prompt("Upper Drawer", 'COMBOBOX', 0, UPPER_DRW_PLACEHOLDER_LIST)
...
class PROMPTS_Drawer_Prompts(sn_types.Prompts_Interface):
    ...
    upper_drawer:  EnumProperty(name="Upper Drawer",
                                items=UPPER_DRW_PLACEHOLDER)
            ...
            for i in range(1,drawer_quantity.get_value()):
                ...
                if(drawer_height and lock_drawer and file_rail_type and file_rail_direction and use_file_rail and extra_deep_drawer_box):
                    ...
                    if drawer_height and drawer_quantity.get_value() >= i:
                        ...
                        col_1 = row.column()
                        if not is_dbl_jwl and props.closet_defaults.use_32mm_system:
                            col_1.label(text="Drawer " + str(i) + " Height:")
                            col_1.prop(self,'drawer_' + str(i) + '_height',text="")
                        elif is_dbl_jwl and props.closet_defaults.use_32mm_system:
                            col_1.label(text="Upper Drawer " + str(i) + " Height:")
                            col_1.prop(self,'upper_drawer',text="")
                            col_1.label(text="Lower Drawer " + str(i) + " Height:")
                            col_1.prop(self,'drawer_' + str(i) + '_height',text="")
                        else:
                        ...
                        if(drawer_height and six_hole and seven_hole):
                            if(drawer_height.get_value() >= six_hole.get_value() and drawer_height.get_value() <= seven_hole.get_value() and self.assembly.obj_y.location.y >= sn_unit.inch(15.99)):
                                ...
                                db_jwl_conditions = drw_H == f7H and depth_in <= 24 and width_in <= 33
                                if not has_ji_ins and not has_jwl_ins and db_jwl_conditions:
                                    ...
                                    if width_in in [18,21,24] and depth_in >= 16 and not is_dbl_jwl:
                                    ...
                                    if is_dbl_jwl and jwlry_ins and width_in >= 18 and width_in < 21 and not has_sld_ins: 
                                        ...
                                        both_velvet = is_upper_velvet and is_lower_velvet
                                        col_4.label(text=f'Jewelry Insert')
                                        ...
                                        if (width_in != 18 or depth_in > 16) and not both_velvet:
                                            col_5 = row.column(align=True)
                                            explain = ''
                                            if not is_upper_velvet and not is_lower_velvet:
                                                explain = '- (Both)'
                                            elif is_upper_velvet and not is_lower_velvet:
                                                explain = '- (Lower)'
                                            elif not is_upper_velvet and is_lower_velvet:
                                                explain = '- (Upper)'
                                        ...
                                        if (width_in != 21 or depth_in > 16) and not both_velvet:
                                            col_5 = row.column(align=True)
                                            explain = ''
                                            if not is_upper_velvet and not is_lower_velvet:
                                                explain = '- (Both)'
                                        ...
                                        if (width_in != 24 or depth_in > 16) and not both_velvet:
                                            col_5 = row.column(align=True)
                                            explain = ''
                                            if not is_upper_velvet and not is_lower_velvet:
                                                explain = '- (Both)'
