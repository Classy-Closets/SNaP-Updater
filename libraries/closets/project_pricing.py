        ...
        for i in range(len(MATERIAL_PARTS_LIST)):
            ...
            if MATERIAL_PARTS_LIST[i][1][:2] in material_types:
                ...
                if 'LF' in MATERIAL_PARTS_LIST[i][13]:
                    ...
                    sheet1["L" + str((i + 1) + 1)] = get_linear_footage(eb_length)
                    sheet1["O" + str((i + 1) + 1)] = (float(MATERIAL_PARTS_LIST[i][11]) * int(MATERIAL_PARTS_LIST[i][7])) * get_linear_footage(eb_length)   #CALCULATED_PRICE
...
def generate_franchise_parts_list():
    ...
    if len(MATERIAL_PARTS_LIST) != 0:
        ...
        for i in range(len(MATERIAL_PARTS_LIST)):
            ...
            sheet1["I" + str((i + 1) + 1)] = str(MATERIAL_PARTS_LIST[i][8]) + " x " + str(MATERIAL_PARTS_LIST[i][9])      #PART_DIMENSIONS           
            sheet1["J" + str((i + 1) + 1)] = MATERIAL_PARTS_LIST[i][10]                                         #THICKNESS
            sheet1["M" + str((i + 1) + 1)] = float(MATERIAL_PARTS_LIST[i][16])                                  #FRANCHISE_PRICE
            sheet1["N" + str((i + 1) + 1)] = float(MATERIAL_PARTS_LIST[i][15])                                  #LABOR
            
            if MATERIAL_PARTS_LIST[i][1][:2] in material_types:
                if 'SF' in MATERIAL_PARTS_LIST[i][13]:
                    sheet1["K" + str((i + 1) + 1)] = get_square_footage(float(MATERIAL_PARTS_LIST[i][8]), float(MATERIAL_PARTS_LIST[i][9]))
                    sheet1["O" + str((i + 1) + 1)] = (((float(MATERIAL_PARTS_LIST[i][16]) * int(MATERIAL_PARTS_LIST[i][7])) * get_square_footage(float(MATERIAL_PARTS_LIST[i][8]), float(MATERIAL_PARTS_LIST[i][9])))  + float(MATERIAL_PARTS_LIST[i][15]))   #CALCULATED_PRICE
                if 'LF' in MATERIAL_PARTS_LIST[i][13]:
                    if MATERIAL_PARTS_LIST[i][14] is not None:
                        eb_length = get_eb_measurements(MATERIAL_PARTS_LIST[i][14], float(MATERIAL_PARTS_LIST[i][8]), float(MATERIAL_PARTS_LIST[i][9]))
                    sheet1["L" + str((i + 1) + 1)] = get_linear_footage(eb_length)
                    sheet1["O" + str((i + 1) + 1)] = (float(MATERIAL_PARTS_LIST[i][16]) * int(MATERIAL_PARTS_LIST[i][7])) * get_linear_footage(eb_length)   #CALCULATED_PRICE
...
def get_pricing_info(sku_num, qty, length_inches=0.0, width_inches=0.0, style_name=None, is_glaze=False, glaze_style=None, glaze_color=None, part_name=None, eb_orientation=None):
        ...
        if style_name is not None:
            ...
            for index, data_row in wood_panel_data_frame.iterrows():
                if data_row['STYLE'] in style_name and style_type == data_row['TYPE']:
                    ...
                    print(lf_molding)
                    print(sf_center_panel)
                    print(molding_price)
                    print(ap_molding_price)
                    print(three_quarter_mdf_price[0])
                    print(wood_labor_price[0])
                    print('---------------------------------------')
                    print(sf_door)
                    print(lf_glaze)
                    print(primer_price[0])
                    print(retail_price)
                    print(glaze_labor[0])
                    print((topcoat_price[0] + catalyst_price[0]))
    ...
    return str(retail_price), str(franchise_price), display_name, vendor_name, vendor_item, style_name, r_labor_price, f_labor_price, uom
    ...
    for item in root.findall("Item"):
        ...
        for part in item.findall("Part"):
            ...
            for part_name in part.findall("Name"):
                ...
                # Added to calculate Wood Panel door styles
                    ...
                    # for sku_value in label.findall("Value"):
                        if sku_value is not None:
                            if sku_value[:2] in material_types:
                                ...
                                if style_name is not None and sku_value[:2] in 'EB':
                                    continue
                                if style_name is not None and sku_value[:2] not in 'EB':
                                    pricing_info = get_pricing_info(SKU_NUMBER, QUANTITY, LENGTH, WIDTH, style_name, is_glaze, glaze_style.text, glaze_color.text, PART_NAME)
                                    if is_glaze:
                                    ...
                                    MATERIAL_PARTS_LIST.append([DESCRIPTION, SKU_NUMBER, pricing_info[3], pricing_info[4], PART_LABEL_ID, pricing_info[2], PART_NAME, QUANTITY, LENGTH, WIDTH, THICKNESS, pricing_info[0], pricing_info[6], pricing_info[8], eb_orientation, pricing_info[7], pricing_info[1]])
                            ...
                            if sku_value[:2] in accessory_types:
                                ...
                                if 'rod'.upper() in PART_NAME.upper():
                                    ...
                                    LENGTH = 0
                                    pricing_info = get_pricing_info(SKU_NUMBER, QUANTITY)
                                if 'pull'.upper() in pricing_info[1] or 'kn'.upper() in pricing_info[1]:
